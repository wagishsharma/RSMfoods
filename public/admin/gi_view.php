...
function rsearch($text)
{
	...
	$qcse = "SELECT * FROM `tlb_cms` WHERE (tlb_cms_title  LIKE '%$text%')";
  ...
  <tr class="thead">
...

	{
		...
		<tr BGCOLOR="'.$bg.'">
		  ...
		  '<td width="75px"><a title="File Edit" href="javascript:void(0);" onclick="PopupCenter(\'cedittext.php?id='.$row['tlb_cms_id'].'\', \'myPope'.$inc.'\',800,600);">Edit</a> | 
		  <a title="Delete File" href="cdelete.php?id='.$row['tlb_cms_id'].'&action=gi" onClick="return delconf('.$row['tlb_cms_id'].');">Delete</a></td>
		</tr>';
		$inc=$inc+1;
	}
	echo'</table>';
...
if(isset($_POST['scsearch']) || (isset($_GET['search']) && !empty($_GET['search'])))
{
	if(isset($_POST['scsearch']))
	{
		if(empty($_POST['csearch']))
		{
			...
			$qe = "SELECT * FROM tlb_cms where tlb_cms_key = 17"; //qe = search with empty query
