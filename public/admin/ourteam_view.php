  ...
  <tr class="thead">
...

	{
		...
		<tr BGCOLOR="'.$bg.'">
		  ...
		  <a title="Delete File" href="cdelete.php?id='.$row['tlb_cms_id'].'&action=ourteam" onClick="return delconf('.$row['tlb_cms_id'].');">Delete</a></td>
		</tr>';
		$inc=$inc+1;
	}
	echo'</table>';
...
if(isset($_POST['scsearch']) || (isset($_GET['search']) && !empty($_GET['search'])))
{
	if(isset($_POST['scsearch']))
	{
		if(empty($_POST['csearch']))
		{
			...
			$qe = "SELECT * FROM tlb_cms where tlb_cms_key = 8"; //qe = search with empty query
