...
<table align="center">
<tr>
<td><font size="+2"><a href="export.php">Dowload Excel</a></font></td>
</tr>
</table>
...
function rsearch($text)
{
	...
	$qcse = "SELECT * FROM `package` WHERE (name  LIKE '%$text%')";
	...
	{
		$qcsa = "SELECT * FROM pakage WHERE MATCH(message) AGAINST('%$text%')"; //qcsa = query customer search any
...
function show_rsearch_data($r,$stext)
{
  ...
  <tr class="thead">
...

	{
		...
		<tr BGCOLOR="'.$bg.'">
    	  <td>'.$inc.'</td>
		  <td>'.substr($row['comefrom'],0,200).'</td>
		  <td>'.substr($row['name'],0,200).'</td>
		  <td>'.substr($row['email'],0,200).'</td>
		  <td>'.$row['subject'].'</td>
		  <td>'.$row['message'].'</td>
		  './/<td><img src="upload/contact/'.$row['contact_img_url'].'" width="100" height="100"/> </td>
		  
		  '<td width="75px"> | 
		  <a title="Delete File" href="pack.php?id='.$row['id'].'&action=enquirey" onClick="return delconf('.$row['id'].');">Delete</a></td>
		</tr>';
		$inc=$inc+1;
	}
	echo'</table>';
...
if(isset($_POST['scsearch']) || (isset($_GET['search']) && !empty($_GET['search'])))
{
	if(isset($_POST['scsearch']))
	{
		if(empty($_POST['csearch']))
		{
			...
			$qe = "SELECT * FROM `package` where `key` = 40"; //qe = search with empty query
