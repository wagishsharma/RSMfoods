...
if(isset($_POST['submit']))
{
	...
	{  
				...
				if($_SESSION['fload'])
				{
					...
					if( $home_image == '' ){
						         $home_image = '';
							     $q = "INSERT INTO `tlb_cms` (`tlb_cms_id` ,`tlb_cms_title`,`tlb_cms_text`,`tlb_cms_key`)VALUES (NULL , '$title','$text',7)";
                    ...
                    if(in_array($ext, $allowedExts) && in_array($_FILES["home_image"]["type"], $permitted))
                    {
                       if($_FILES["home_image"]["error"] > 0){ $flag = 'error'; }
                             ...
                             if(file_exists("upload/" . $_FILES["home_image"]["name"])){
							     ...
							     $q = "INSERT INTO `tlb_cms` (`tlb_cms_id` ,`tlb_cms_title`, `tlb_cms_url`,`tlb_cms_text`,  `tlb_cms_key`)VALUES (NULL , '$title', '$home_image','$text', 7)";
							     $r = mysqli_query($dbc,$q);  
                             }
                       }
                    }
					...
					if(!empty($r))
					{
						...
						echo'<span class="successmssg">Content succeessfully added to the website, <br/> <a href=index.php?option=contact_add>click to add more</a> <b></span>';
