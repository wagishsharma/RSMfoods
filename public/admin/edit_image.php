			...
			foreach($_POST as $key=> $value){
			   ...
			   $_POST[$key] = trim($value);
					...
					if(!empty($file)){
					     ...
					     $permitted = array('image/jpeg','image/png','image/gif','image/jpg','image/JPG');
                         ...
                         if(in_array($ext, $allowedExts) && in_array($_FILES["file"]["type"], $permitted))
                         {
                           if($_FILES["file"]["error"] > 0){ $flag = 'error'; }
							     ...
							     $q = "UPDATE `tlb_cms` SET tlb_cms_url ='$name' WHERE tlb_cms_id = '$_GET[id]' LIMIT 1";  
					  ...
					  $q = "UPDATE `tlb_cms` SET tlb_cms_text ='$text' WHERE tlb_cms_id = '$_GET[id]' LIMIT 1";
					  $r = mysqli_query($dbc,$q); 
					}<?php */
			        
			if(!empty($r)){
				echo'<span class="successmssg">Details Succeessfully Updated, <a href="javascript:window.close();">Click here to continue</a></span>';
...
<form action="" method="post" name="add_com_mem" class="form" enctype="multipart/form-data">
  <table width="730" border="0" cellspacing="5" cellpadding="5" style="margin-left:50px;">
    <tr>
      ...
      <td><input name="file" type="file" style="width:250px;" maxlength="50" /></td>
    </tr>
<!--     <tr>
