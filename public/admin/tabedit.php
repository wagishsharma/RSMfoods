...
{
	$fclick = true;
	if(!empty($_POST['title']) && !empty($_POST['programme']) && !empty($_POST['duration']) && !empty($_POST['eligibility']))
	{   
	    if($_SESSION['fload']){
			...
			foreach($_POST as $key=> $value){
			   ...
			   $_POST[$key] = trim($value);
				    ...
				    $all = $programme.'//'.$duration.'//'.$eligibility; 
			$r = ''; $flag = '';
			include_once('include/conectdb.php');
			$q = "UPDATE tlb_saheed SET tlb_cms_title = '$_POST[title]',  tlb_cms_text = '$all' WHERE tlb_cms_id = '$_GET[id]' LIMIT 1"; 
...
else
{
	...
	include_once('include/conectdb.php');
	$q = "SELECT * FROM tlb_saheed WHERE tlb_cms_id = $_GET[id] LIMIT 1";
	$r = mysqli_query($dbc, $q);
	if(mysqli_num_rows($r) == 1)
	{
		$row = mysqli_fetch_assoc($r); 
		$ar = explode('//', $row['tlb_cms_text']);
		$_POST['title'] = $row['tlb_cms_title'];
		$_POST['programme'] = $ar['0'];
		$_POST['duration'] = $ar['1'];
		$_POST['eligibility'] = $ar['2'];
...
<form action="" method="post" name="add_com_mem" class="form" enctype="multipart/form-data">
  <table width="730" border="0" cellspacing="5" cellpadding="5" style="margin-left:50px;">
    <tr>
      <td><strong>Project Title:</strong> <span class="star style1">*</span></td>
      <td><input name="title" type="text" style="width:400px;" value="<?php if(isset($_POST['title'])) { echo $_POST['title'];} else echo''; ?>" maxlength="100" /><?php if($fclick){if(empty($_POST['title']))echo'<span class="error"> Title is missing</span>';}?></td>
    </tr>
     <tr>
      <td><strong>Programme:</strong> <span class="star style1">*</span></td>
      <td><input name="programme" type="text" style="width:400px;" value="<?php if(isset($_POST['programme'])) { echo $_POST['programme'];} else echo''; ?>" maxlength="100" /><?php if($fclick){if(empty($_POST['programme']))echo'<span class="error"> Title is missing</span>';}?></td>
    </tr>
    <tr>
      <td><strong>Duration:</strong> <span class="star style1">*</span></td>
      <td><input name="duration" type="text" style="width:400px;" value="<?php if(isset($_POST['duration'])) { echo $_POST['duration'];} else echo''; ?>" maxlength="100" /><?php if($fclick){if(empty($_POST['duration']))echo'<span class="error"> Title is missing</span>';}?></td>
    </tr>
    <tr>
      <td><strong>Eligibility:</strong> <span class="star style1">*</span></td>
      <td><input name="eligibility" type="text" style="width:400px;" value="<?php if(isset($_POST['eligibility'])) { echo $_POST['eligibility'];} else echo''; ?>" maxlength="100" /><?php if($fclick){if(empty($_POST['eligibility']))echo'<span class="error"> Title is missing</span>';}?></td>
    </tr>
    <tr>
      <td colspan="2">&nbsp;</td>
    </tr>
