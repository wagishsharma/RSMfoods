...
function rsearch($text)
{
	...
	$qcse = "SELECT * FROM job WHERE ((post_name  LIKE '%$text%')  || (job_des  LIKE '%$text%'))  ";
	...
	{
		$qcsa = "SELECT * FROM job WHERE MATCH(`post_name` ,`company_logo`,`job_desc`) AGAINST('%$text%')"; //qcsa = query customer search any
...
function show_rsearch_data($r,$stext)
{
  ...
  <tr class="thead">
    <td width="30">S.No</td>
	<td width="200">Job Post Name</td>
	<td width="150">Company Logo</td>
	<td width="450">Job Description</td>
	<td width="450">Company Name</td>
	<td width="300">Job Type</td>
	<td width="300">City</td>
	<td width="300">Job Location</td>

	<td width="200">Options</td>
  </tr>';
	$inc=1;
	while($row=mysqli_fetch_assoc($r))
	{
		...
		<tr BGCOLOR="'.$bg.'">
    	  <td>'.$inc.'</td>
		  <td>'.substr($row['post_name'],0,200).'</td>
		  <td width="12%"><img src="upload/'.$row['company_logo'].'" width="100" height="100" /></td>
		   <td>'.substr($row['job_des'],0,200).'</td>
		   <td>'.substr($row['cname'],0,200).'</td>
		   <td>'.substr($row['jobtype'],0,200).'</td>
		   <td>'.substr($row['city'],0,200).'</td>
		   <td>'.substr($row['location'],0,200).'</td>
		  './/<td><img src="upload/contact/'.$row['contact_img_url'].'" width="100" height="100"/> </td>
		  '<td width="75px"><a title="File Edit" href="javascript:void(0);" onclick="PopupCenter(\'edit_job.php?id='.$row['id'].'\', \'myPope'.$inc.'\',800,600);">Edit</a> | 
		  <a title="Delete File" href="delete_img.php?id='.$row['id'].'&action=job" onClick="return delconf('.$row['id'].');">Delete</a></td>
		</tr>';
		$inc=$inc+1;
	}
	echo'</table>';
...
if(isset($_POST['scsearch']) || (isset($_GET['search']) && !empty($_GET['search'])))
{
	if(isset($_POST['scsearch']))
	{
		if(empty($_POST['csearch']))
		{
			...
			$qe = "SELECT * FROM `job` where `key`=27"; //qe = search with empty query
