	...
	{   
	    if($_SESSION['fload']){
			...
			if(!empty($_FILES['image']['name'])){
			   ...
			   $selectQ = "select tlb_cms_url from tlb_cms where tlb_cms_id = '$_GET[id]' LIMIT 1";
               ...
               if(in_array($ext, $allowedExts) && in_array($_FILES["image"]["type"], $permitted)){
                 if($_FILES["image"]["error"] > 0){ $flag = 'error'; }
					     ...
					     // echo 'hello';
						  ...
						  $q = "UPDATE tlb_cms SET tlb_cms_title = '$_POST[title]',    tlb_cms_url = '$uImage' WHERE tlb_cms_id = '$_GET[id]' LIMIT 1";
					      $r = mysqli_query($dbc,$q); 
						  unlink('upload/'.$row[0]); 
                 }
                }else{ $flag = 'invalid'; }					   
			}else{ 
			   $q = "UPDATE tlb_cms SET tlb_cms_title = '$_POST[title]',  tlb_cms_text = '$_POST[text]' WHERE tlb_cms_id = '$_GET[id]' LIMIT 1"; 
			   $r = mysqli_query($dbc,$q);			
			}
...
<form action="" method="post" name="add_com_mem" class="form" enctype="multipart/form-data">
  <table width="730" border="0" cellspacing="5" cellpadding="5" style="margin-left:50px;">
    ...
    <tr>
      <td><strong>Upload Image:</strong><span class="star style2">*</span></td>
