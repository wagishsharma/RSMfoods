...
if(isset($_POST['submit']))
{
	...
	{  		
				if($_SESSION['fload'])
				{
					...
					$flag = ''; $r = '';
				    $q = "INSERT INTO `tlb_cms`(`tlb_cms_id`, `tlb_cms_title`, `tlb_cms_text`, `tlb_cms_key`) VALUES (null, '$title', '$text', 8)";
				    $r = mysqli_query($dbc,$q);  
					if(!empty($r))
					{
						echo'<span class="successmssg">Content succeessfully added to the website, <br/> <a href=index.php?option=ourteam_add>Click to add more</a> <b></span>';
...
<form action="" method="post" name="news" class="form" enctype="multipart/form-data">
  <table width="830" border="0" cellspacing="5" cellpadding="5" style="margin-left:50px;">
    <tr>
      ...
      <td><input name="title" type="text" style="width:250px;" value="<?php if(isset($_POST['title'])) { echo $_POST['title'];} else echo''; ?>" maxlength="500" /><?php if($fclick){if(empty($_POST['title']))echo'<span class="error"> Title is missing</span>';}?></td>
    </tr>
     <tr>
