				...
				if($_SESSION['fload'])
				{
                    ...
                    if(in_array($ext, $allowedExts) && in_array($_FILES["file"]["type"], $permitted))
                    {
                       if($_FILES["file"]["error"] > 0){ $flag = 'error'; }
							     ...
							     $q = "INSERT INTO `tlb_cms`(`tlb_cms_id`, `tlb_cms_url`, `tlb_cms_key`) VALUES (null, '$name', 6)";
					...
					if(!empty($r))
					{
						...
						echo'<span class="successmssg">Content succeessfully added to the website, <br/> <a href=index.php?option=clogo_add>Click to add more</a> <b></span>';
