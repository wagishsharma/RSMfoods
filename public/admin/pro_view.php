...
function rsearch($text)
{
	...
	$qcse = "SELECT * FROM `tlb_saheed` WHERE (tlb_cms_text  LIKE '%$text%')";
	//qcse = query customer search exact
	$rcse = mysqli_query($dbc,$qcse);
	if(mysqli_num_rows($rcse)>0)
	{
		echo'<p style="margin-left:20px;color:red;">'.$output.'Matching item are : <strong>'.mysqli_num_rows($rcse).'</strong></p>';
		show_rsearch_data($rcse,$text);
	}
	else
	{
		$qcsa = "SELECT * FROM tlb_cms WHERE MATCH(tlb_cms_text) AGAINST('%$text%')"; //qcsa = query customer search any
		$rcsa = mysqli_query($dbc,$qcsa);
		if(mysqli_num_rows($rcse)>0)
		{
			echo'<p style="margin-left:20px;color:red;">'.$output.'Matching Text found are: <strong>'.mysqli_num_rows($rcsa).'</strong></p>';
  ...
  <tr class="thead">
...

	{
		...
		<tr BGCOLOR="'.$bg.'">
		  ...
		  <td width="12%"><a href="upload/'.$row['tlb_cms_url'].'" width="100" height="100"><img src="upload/downloadss.jpg"/></a></td> 
		  <td width="15%"><a title="File Edit" href="javascript:void(0);" onclick="PopupCenter(\'edit_image.php?id='.$row['tlb_cms_id'].'\', \'myPope'.$inc.'\',800,600);">Edit</a> |
		  <a title="Delete File" href="delete_img.php?id='.$row['tlb_cms_id'].'&action=about" onClick="return delconf('.$row['tlb_cms_id'].');">Delete</a> 
		 </td>
		</tr>';
		$inc=$inc+1;
	}
...
if(isset($_POST['scsearch']) || (isset($_GET['search']) && !empty($_GET['search'])))
{
	if(isset($_POST['scsearch']))
	{
		if(empty($_POST['csearch']))
		{
			...
			$qe = "SELECT * FROM tlb_cms where tlb_cms_key = 15"; //qe = search with empty query
			$re = mysqli_query($dbc, $qe);
			if(mysqli_num_rows($re) == 0)
			{
				echo'<span class="warn">Sorry database is empty.</span>';
			}
			else
			{
				echo'<p style="margin-left:20px;color:red;">Total text available are : <strong>'.mysqli_num_rows($re).'</strong></p>';
	...
	else
	{
		...
		{  
			echo'<p style="margin-left:20px;color:red;">Total Title And Text Are : <strong>'.mysqli_num_rows($re).'</strong></p>';
			show_rsearch_data($re,'ALL');
		}
	}
}
