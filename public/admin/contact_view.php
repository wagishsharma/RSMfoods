  ...
  <tr class="thead">
	...
	while($row=mysqli_fetch_assoc($r))
	{
		...
		<tr BGCOLOR="'.$bg.'">
		  ...
		  <a title="Delete File" href="delete_img.php?id='.$row['tlb_cms_id'].'&action=contact" onClick="return delconf('.$row['tlb_cms_id'].');">Delete</a></td>
		</tr>';
		$inc=$inc+1;
	}
	echo'</table>';
...
if(isset($_POST['scsearch']) || (isset($_GET['search']) && !empty($_GET['search'])))
{
	if(isset($_POST['scsearch']))
	{
		if(empty($_POST['csearch']))
		{
			...
			$qe = "SELECT * FROM tlb_cms where tlb_cms_key=7"; //qe = search with empty query
