				...
				if($_SESSION['fload'])
				{
                    ...
                    if(in_array($ext, $allowedExts) && in_array($_FILES["file"]["type"], $permitted))
                    {
                       if($_FILES["file"]["error"] > 0){ $flag = 'error'; }
							     ...
							     $q = "INSERT INTO `tlb_cms`(`tlb_cms_id`,`tlb_cms_title`, `tlb_cms_url`, `tlb_cms_key`) VALUES (null,'$title', '$name', 26)";
					...
					if(!empty($r))
					{
						...
						echo'<span class="successmssg">Content succeessfully added to the website, <br/> <a href=index.php?option=emp_add>Click to add more</a> <b></span>';
...
<form action="" method="post" name="news" class="form" enctype="multipart/form-data">
  ...
  <tr>
      ...
      <td><select name="title" style="width:250; height:30px;" required="required">
      <option value="">---Select---</option>
      <option value="PROVIDING PREMIUM ">PROVIDING PREMIUM </option>
      <option value="STRIVING FOR INCLUSIVE  ">STRIVING FOR INCLUSIVE  </option>
      <option value="PROMOTING SUSTAINABLE  ">PROMOTING SUSTAINABLE  </option>
      <option value="PROMOTING THE MODERNIZATION  ">PROMOTING THE MODERNIZATION </option>
      <option value="USE OF INFORMATION SENSING  ">USE OF INFORMATION SENSING</option>
